...
contract LiraeToken is ERC20Interface, Owned, SafeMath {
    ...
    function buyTokens(address beneficiary) public payable {
        ...
        if (weiAmount >= 1 finney) {
            // update state
            weiRaised = safeAdd(weiRaised, weiAmount);

            // calculate token amount to be created
            uint256 weiTokens = safeDiv(weiAmount, 1 finney);
            uint256 tokensIssued = safeMul(weiTokens, _rate);

            // transfer tokens
            balances[owner] = safeSub(balances[owner], tokensIssued);
            balances[beneficiary] = safeAdd(balances[beneficiary], tokensIssued);

            Transfer(owner, beneficiary, tokensIssued);
            forwardFunds(weiAmount);
        }
    }
